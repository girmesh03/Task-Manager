...
- **Three Task Types**:
  - **ProjectTask**: Outsourced to external vendors with costs
...
- **Assignees & Watchers**: Multiple users per task (max 20 each, watchers HOD only on ProjectTask). There is no assignees on ProjectTask and RoutineTask.
- **Tags**: Categorize tasks (max 5 per task)
- **Cost Tracking**: Estimated vs actual costs with history (ProjectTask only)
- **Vendor Association**: ProjectTask must have vendor for external client taking the task
...
- **Assignee**: `assignee` field (required) - Single user or array of users
...
- **Organization Scoped**: Must belong to organization and must not scoped to department
