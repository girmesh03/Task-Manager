...
- **Components**: PascalCase `.jsx` (e.g., `CreateUpdateUser.jsx`)
...
- ProjectTask → Watchers (many-to-many, watchers HOD only)
- AssignedTask → Assignees/Materials (via TaskActivity, many-to-many with quantity)
...
**Organization**: name, description, industry, address, phone, email, timestamps

**Department**: name, description, organization, timestamps

**User**: firstName, lastName, email, password, role, department, organization, employeeId, position, phone, profilePicture, skills, status, joinedAt, emailPreferences, passwordResetToken, passwordResetExpires, isPlatformUser, isHod, lastLogin, timestamps

**BaseTask**: title, description, status, priority, watchers, tags, createdBy, updatedBy, department, organization, taskType (discriminator), timestamps. Abstract base holding shared fields for all task types.

**ProjectTask** (extends BaseTask): vendor (required), startDate, dueDate, estimatedCost, actualCost, currency, costHistory. Outsourced to external vendor. Materials added via TaskActivity.

**RoutineTask** (extends BaseTask): materials (direct), date, timestamps. Daily routine task received from outlets. Materials added directly (no TaskActivity). Restricted status/priority.

**AssignedTask** (extends BaseTask): assignee (single or many), startDate, dueDate. Assigned to department user(s). Materials added via TaskActivity.

**TaskActivity**: content, parentId (ProjectTask or AssignedTask ONLY), parentModel, materials, createdBy, updatedBy, department, organization, timestamps. Does NOT exist for RoutineTask.

**TaskComment**: content, parentId, parentModel, mentions, createdBy, updatedBy, department, organization, timestamps

**Material**: name, description, category, quantity, unitType, cost, price, currency, vendorId, department, organization, timestamps

**Vendor**: name, description, contactPerson, email, phone, address, department, organization, timestamps. External client who takes and completes ProjectTasks.

**Attachment**: filename, fileUrl, fileType, fileSize, parent, parentModel, uploadedBy, department, organization, timestamps

**Notification**: title, message, type, isRead, recipient, entity, entityModel, organization, expiresAt, timestamps
